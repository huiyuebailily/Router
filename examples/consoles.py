...
class Console( Frame ):
    ...
    def __init__( self, parent, net, node, height=20, width=64, title='Node' ):
...
class ConsoleApp( Frame ):
    ...
    def createConsoles( self, parent, nodes, width, title ):
        ...
        for node in nodes:
            ...
            #row = index / width  Huiyue bai 2022-03-23
            row = int(index / width) #  Huiyue bai 2022-03-23
            column = index % width
            print("row =", row ,"column=",column,"index =",index ,"width=",width)
...
if __name__ == '__main__':
    ...
    network = TreeNet( depth=1, fanout=2 )
    network.start()
    #app = ConsoleApp( network, width=4 )
    app = ConsoleApp( network, width=2 )
    app.mainloop()
    network.stop()
